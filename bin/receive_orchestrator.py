...
def load_passes_to_schedule(config: Dict[str, Any]) -> List[Dict[str, Any]]:
    """Load passes scheduled in the near future.

    For now, this reads from pass schedule files.
    In the future, could read from a database or calendar.
    ...
    generated_units_dir = os.path.join(output_dir, "generated_units")

    if not os.path.isdir(generated_units_dir):
        return passes
    ...
    # Look for pass JSON files in the next 24 hours
    now = utc_now()
    deadline = now + timedelta(hours=24)

    try:
        for filename in os.listdir(generated_units_dir):
            if not filename.endswith(".pass.json"):
                continue

            pass_file = os.path.join(generated_units_dir, filename)
            try:
                with open(pass_file, "r", encoding="utf-8") as f:
                    pass_data = json.load(f)

                # Check if pass is in the near future
                pass_start = pass_data.get("pass_start")
                if pass_start:
                    # Parse ISO format: "2026-05-11T03:11:56Z"
                    pass_dt = datetime.fromisoformat(pass_start.replace("Z", "+00:00"))
                    if now <= pass_dt <= deadline:
                ...
                logger.warning("Failed to parse pass file %s: %s", pass_file, e)

    except OSError as e:
        logger.warning("Failed to read generated_units directory: %s", e)
...
def receive_pass(
    ...
    """Run receive_pass.py for the given pass.
    ...
    # Build receive_pass.py command
    python_bin = config.get("paths", {}).get("python_bin", "python3")
    pass_file = pass_data.get("pass_file", "")

    cmd = [python_bin, script]
    if pass_file:
        cmd.append(pass_file)
    ...
    # Extract pass_output_dir from stdout or construct from pass data
    # For now, construct it from satellite and timestamp
    satellite = pass_data.get("satellite", "UNKNOWN").replace(" ", "_")
    pass_start = pass_data.get("pass_start", "")

    if pass_start:
        # Convert ISO format to local format: 2026-05-11T03:11:56Z → 2026-05-11_03-11-56
        try:
            dt = datetime.fromisoformat(pass_start.replace("Z", "+00:00"))
            local_dt = dt.astimezone()
            timestamp = local_dt.strftime("%Y-%m-%d_%H-%M-%S")
            pass_output_dir = os.path.join(
                config.get("paths", {}).get("output_dir", ""),
                f"{timestamp}_{satellite}",
            )
            pass_output_dir = os.path.expanduser(pass_output_dir)
...
def process_scheduled_passes(config: Dict[str, Any]) -> int:
    ...
    for i, pass_data in enumerate(passes, 1):
        ...
        try:
            ...
            logger.warning("Could not parse pass_start time, proceeding immediately")
